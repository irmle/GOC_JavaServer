 ...
 * 업뎃날짜 :
 * 목    적 :
 *      유저가 바리케이드 설치 RMI를 호출했을 때, 서버에서 처리되는 매서드이다.
 *      넘겨받은 파라미터값을 사용하여
 *          1. 유저가 속한 월드를 찾고
 *          2. 바리케이드 설치 요청 정보를 담은 Action 객체를 생성하고
 *          3. 생성한 Action 객체를 1에서 찾은 월드 내 액션 큐에 집어넣는다
 */
public class Logic_installBarricade {

    //Called_RMI(Remote Method Invocation)_Method
    public static void RMI_Packet(RMI_ID rmi_id, short rmi_ctx, int worldMapID, int userEntityID, int barricadeType, int areaNumber) {

        // 월드 내 건설 가능한 지점 목록을 담고 잇는 빌드슬롯 목록 중, 어느 슬롯에 대한 건설 요청인지??
        int buildSlotNum = areaNumber;

        barricadeType = BuildType.BARRIER;
