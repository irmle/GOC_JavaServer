...
import Network.RMI_Classes.RMI_InitClasses.*;
import Network.RMI_LogicMessages.*;
...
//=======================================================================================
    ...
    public static int rmi_protocol_version = 2020052801;
...
//=======================================================================================
    ...
    public static void OnDisConnected(ChannelHandlerContext ctx)
    {
        ...
        if(DisconnectID != null)
        {
            ...
            NetworkManager.pushUDPChannel( udpChannel );

            //최종적으로 접속 종료시 1감소.
            NetworkManager.decrementUserConnection();
            ...
            if(UDP_Sender == null)
            {
                switch (packetType)
                {
                    case RMI_ConnectionPacketType.RMI_ProtocolVersionCheck: //Protocol Version 체크시.
                        ...
                        sendData.base64Encoded_publicKey = RMI_EncryptManager.getPublicKey();
                        //고정된 대칭키로 암호화!
                        byte[] sendData_ = RMI_EncryptManager.encryptAES_256(sendData.getBytes(), "RMI_Connection_Protocol", "RMI_Connection_Protocol");
                        sendData = null;
                        //보낼 데이터 만큼 directBuffer 할당.
                        ByteBuf RMI_RSA_PublicKey = alloc.directBuffer(32768);
                    ...
                    case RMI_ConnectionPacketType.RMI_Send_EncryptedAES_Key: //클라로부터, 서버가 보낸 공개키에 의해 암호화된 AES대칭키 데이터 수신시.
                        ...
                        try
                        {
                            ...
                            byte[] recvPacket1 = RMI_EncryptManager.decryptRSA_private(data, RMI_EncryptManager.getPrivateKey());
                        ...
                        if(recvData1 != null)
                        {
                            ...
                            //접속에 성공한 클라이언트수 증가.
                            NetworkManager.incrementUserConnection();
                            ...
                            //서버의 공용 AES키를 지정.
                            send_EncryptedAccept_Data.AESEncrypted_PublicAESKey = RMI_EncryptManager.Public_AESKeys.aesKey.getBytes(StandardCharsets.UTF_8);
                            send_EncryptedAccept_Data.AESEncrypted_PublicAESIV = RMI_EncryptManager.Public_AESKeys.aesIV.getBytes(StandardCharsets.UTF_8);
                            ...
                            byte[] encryptPacket = RMI_EncryptManager.encryptAES_256(plainData, newRMI_ID.AESKey.aesKey, newRMI_ID.AESKey.aesIV);
            ...
            if(UDP_Sender != null)
            {
                switch (packetType)
                {
                    case RMI_ConnectionPacketType.RMI_UDP_ConnectionConfirm:
                        ...
                        if(remote != null)
                        {
                            byte[] udpCheck = RMI_EncryptManager.decryptAES_256(data, remote.AESKey.aesKey, remote.AESKey.aesIV);
                            try
                            {
...
//호출되었을 때 실행되는 부분. 로그 기록에 사용되는 용도.
    ...
    //RMI 암호화 관리자 클래스.
    private static final RMI_EncryptManager RMI_ENCRYPT = new RMI_EncryptManager();
    ...
    static void send_OverConnectionAnnounce(ChannelHandlerContext ctx)
    {
        ...
        RMI_OverConnectionAnnounce overConnectionAnnounce = new Network.RMI_Classes.RMI_InitClasses.RMI_OverConnectionAnnounce();
        overConnectionAnnounce.maxConnection = NetworkManager.getClient_Max_Connection();
